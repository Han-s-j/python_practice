...
def fn_lotto(update, context):
    ...
    for i in range(cnt):
        ...
        lotto_num2 = sorted(lotto_num)
        #user_id = update.effective_chat.id
        #context.bot.send_message(chat_id=user_id, text=lotto_num2)
        update.message.reply_text(lotto_num2)
...
def get_photo(update, context):
    img_path2 = './test.png'
    user_id = update.effective_chat.id
    context.bot.send_photo(chat_id=user_id, photo=open(img_path2,'rb'))

# 사진 가져오기
updater.dispatcher.add_handler(CommandHandler('lotto', fn_lotto))
# 버튼
